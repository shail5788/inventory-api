...
module.exports = function validateRegisterInput(data) {
  ...
  if (!Validator.isLength(data.name.firstname, { min: 2, max: 30 })) {
    errors.name = "Name must be between 2 and 30 characters";
  }

  if (Validator.isEmpty(data.name.firstname)) {
    errors.name.firstname = "firstname field is required";
  }
  if (Validator.isEmpty(data.name.lastname)) {
    errors.name.lastname= "lastname field is required";
  }
